        ...
        {patients.map((patient) => (
          <button
            ...
            className={`w-full min-h-[70px] text-left flex items-center gap-2 py-3 rounded-xl transition-all ${
              selectedPatient?.name === patient.name ? "bg-[#D8FCF7]" : "hover:bg-gray-100"
            }`}
          >
            <img src={patient.profile_picture} alt={patient.name} className="img w-12 h-12 rounded-full object-cover flex-shrink-0" />
            <div className="flex-1 min-w-0">
              <h4 className="font-medium text-sm font-bold truncate">{patient.name}</h4>
